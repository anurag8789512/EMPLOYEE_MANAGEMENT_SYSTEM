...
public class viewempdept extends JFrame implements ActionListener{
    
    JTable jt;
    Choice cempid;
    JButton sear,prin,back,update,det,proj,skill,perfor;
    JTextField jta;
    
    viewempdept(){
        
        getContentPane().setBackground(Color.WHITE);
        setTitle("VIEW EMPLOYEE DEPARTMENT");
        setLayout(null);
        ...
        perfor.setBounds(1010,10,140,20);
        ...
        try{
            ...
            Conn c=new Conn();
            ResultSet rs=c.s.executeQuery("select b.name,a.* from emp_detail b,dept a where a.empid=b.empid");
        ...
        if(ae.getSource()==sear){
            ...
            try{
                ...
                if(cempid.getSelectedItem()=="ALL"){
                    ResultSet rs=c.s.executeQuery("select b.name,a.* from dept a,emp_detail b where a.empid=b.empid");
                    jt.setModel(DbUtils.resultSetToTableModel(rs));
                ...
                else if(cempid.getSelectedItem()=="Employee ID")
                {
                    ...
                    ResultSet rs2=c.s.executeQuery("select b.name,a.* from dept a,emp_detail b where a.empid=b.empid and a.empid='"+str+"'");
                    jt.setModel(DbUtils.resultSetToTableModel(rs2));
                }
                else if(cempid.getSelectedItem()=="Name")
                {
                    String str=jta.getText().trim();
                    ResultSet rs2=c.s.executeQuery("select b.name,a.* from dept a,emp_detail b where a.empid=b.empid and b.name='"+str+"'");
                    jt.setModel(DbUtils.resultSetToTableModel(rs2));
                }
                else if(cempid.getSelectedItem()=="Aadhar No")
                {
                    String str=jta.getText().trim();
                    ResultSet rs2=c.s.executeQuery("select b.name,a.* from dept a,emp_detail b where a.empid=b.empid and b.adhno='"+str+"'");
                    jt.setModel(DbUtils.resultSetToTableModel(rs2));
                }
                else
                {
           ...
           jt.print();
               ...
               e.printStackTrace();
                    ...
                    setVisible(false);
                    new Updateempdept(jta.getText().trim());
