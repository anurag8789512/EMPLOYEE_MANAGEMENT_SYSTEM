...
public class viewemp extends JFrame implements ActionListener{
    ...
    JTable jt;
    Choice cempid;
    JButton sear,prin,back,update,dept,proj,skill,perfor;
    JTextField jta;
    ...
    viewemp(){
        ...
        try{
            ...
            Conn c=new Conn();
            ResultSet rs=c.s.executeQuery("select * from emp_detail");
        ...
        if(ae.getSource()==sear){
            ...
            try{
                ...
                else if(cempid.getSelectedItem()=="Employee ID")
                {
                    ...
                    ResultSet rs2=c.s.executeQuery("select * from emp_detail where empid='"+str+"'");
                    jt.setModel(DbUtils.resultSetToTableModel(rs2));
                }
                else if(cempid.getSelectedItem()=="Name")
                {
                    String str=jta.getText().trim();
                    ResultSet rs2=c.s.executeQuery("select * from emp_detail where NAME='"+str+"'");
                    jt.setModel(DbUtils.resultSetToTableModel(rs2));
                }
                else if(cempid.getSelectedItem()=="Aadhar No")
                {
                    String str=jta.getText().trim();
                    ResultSet rs2=c.s.executeQuery("select * from emp_detail where adhno='"+str+"'");
                    jt.setModel(DbUtils.resultSetToTableModel(rs2));
                }
                else
                {
           ...
           jt.print();
               ...
               e.printStackTrace();
                        ...
                        setVisible(false);
                            new Updateemp(jta.getText().trim());
                    }
